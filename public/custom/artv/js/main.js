				...
				//向上移动
					if(page_n != Msize){
						if(status_bool)
						{
							...
							//_audio.src = 'http://m.artvoi.com/resources/assets/image/louvre.mp3';
							_audio.play();
						}
...
J_fTxt.eq(0).show();
	...
	function success(){
		...
		*/							
		 ...
		 * 设置详情png
		 */
		var odata = $(".m-page").eq(page_n-1).find(".animated").attr("odata");
		if(odata){
			$(".m-page").eq(page_n-1).find(".animated").css("backgroundImage","url(http://m.artvoi.com/resources/html5/img/orsay/"+odata+".png)");
		}
